...
/**
 * Supplier Development Report (Dispatch Inquiry) — landscape PDF.
 *
 * Variant of the regular Supplier Development Report for the legacy
 * "Sample Development Report for Dispatch Inquiry" print. Compared to the
 * regular supplier-side PDF this adds a CATEGORY column between
 * FABRIC / CONTENT / GSM and SIZE.
 *
 *   - Top      : "SUPPLIER DEVELOPMENT REPORT" centered.
 *   - Sub-bar  : "FROM : DD/MM/YYYY   TO : DD/MM/YYYY" centered.
 *   - Table    : 12 cols — S#, PICTURE (2 sub-cells), FTY NAME,
 *                STYLE / DESCRIPTION, FABRIC / CONTENT / GSM, CATEGORY,
 *                SIZE, COLORS, QTY, FOB PRICE, DELIVERY DATE, STATUS.
 *
...
const SUPPLIER_SDR_DISPATCH_DEMO = {
  ...
  items: [
    ...
    },
    {
      ...
      fabric: 'jacquard,70% cotton',
      content: '30% polyester',
      gsm: '330 GSM',
      category: 'WOMEN,S',
      size: 'L',
      colors: 'Olive green',
...
export { SUPPLIER_SDR_DISPATCH_DEMO };
...
/**
 * Build the Supplier Development Report (Dispatch) PDF blob.
 *
 * @param {{
 *   items?: object[];
 *   title?: string;
 ...
 * @param {Blob} pdfBlob
 */
export function openSupplierSdrDispatchPdf(mode, pdfBlob) {
  ...
  a.href = blobUrl;
  a.download = 'Supplier-Development-Report-Dispatch.pdf';
  a.rel = 'noopener';
