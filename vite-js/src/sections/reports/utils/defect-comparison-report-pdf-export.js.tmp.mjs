 ...
 *                 (bold, underlined, navy).
 *   - Top-RIGHT : one combined scope box with two columns (Set A | Set B), each
 *                 listing Supplier/Year/Customer/Style/PO# and six total rows.
...
const LOGO_PATH = `${'/'}logo/AMSlogo.png`;
...
const SCOPE_ROW_COUNT = 11;
const SCOPE_ROW_H = 15;
const SCOPE_BOX_H = SCOPE_ROW_COUNT * SCOPE_ROW_H;

// Header positioning (legacy Image 1 proportions — spacing only)
const HEADER_LOGO_W = 90;
const HEADER_LOGO_H = 28;
const HEADER_LOGO_Y = V_MARGIN;
const HEADER_PRINT_DATE_GAP = 8;
const HEADER_TITLE_GAP = 18;
const HEADER_SCOPE_BOX_W_RATIO = 0.5;
const HEADER_TABLE_GAP = 14;
...
function buildScopeColumnLines(scope) {
...
function drawCombinedScopeBox(doc, x, y, w, scopeA, scopeB) {
  const rowsA = buildScopeColumnLines(scopeA);
  const rowsB = buildScopeColumnLines(scopeB);
  const halfW = w / 2;

  strokeRect(doc, x, y, w, SCOPE_BOX_H);
  setBorder(doc);
  doc.line(x + halfW, y, x + halfW, y + SCOPE_BOX_H);

  for (let i = 0; i < SCOPE_ROW_COUNT; i += 1) {
    const rowY = y + i * SCOPE_ROW_H;
    if (i > 0) {
      doc.line(x, rowY, x + w, rowY);
    }
    textInRect(doc, x, rowY, halfW, SCOPE_ROW_H, rowsA[i], {
      align: 'left',
      bold: true,
      fontSize: 7.5,
      pad: 4,
    ...
    textInRect(doc, x + halfW, rowY, halfW, SCOPE_ROW_H, rowsB[i], {
      align: 'left',
      bold: true,
      fontSize: 7.5,
      pad: 4,
...
function drawFirstPageHeader(doc, logoDataUrl, meta) {
  const innerLeft = H_MARGIN;
  const innerW = PAGE_W - 2 * H_MARGIN;
  const logoY = HEADER_LOGO_Y;
  const printDateY = logoY + HEADER_LOGO_H + HEADER_PRINT_DATE_GAP;
  const titleY = printDateY + HEADER_TITLE_GAP;

  if (logoDataUrl) {
    try {
      doc.addImage(logoDataUrl, 'PNG', innerLeft, logoY, HEADER_LOGO_W, HEADER_LOGO_H);
    } catch {
      /* ignore */
    }
  }
  ...
  doc.setFontSize(8);
  doc.setTextColor(0, 0, 0);
  doc.text(`Print Date: ${meta.printDate} ${meta.printTime}`, innerLeft + HEADER_LOGO_W / 2, printDateY, {
    align: 'center',
    baseline: 'alphabetic',
  });
  ...
  doc.text(title, innerLeft, titleY, { align: 'left', baseline: 'alphabetic' });
  const titleW = doc.getTextWidth(title);
  doc.setDrawColor(NAVY[0], NAVY[1], NAVY[2]);
  doc.setLineWidth(0.6);
  doc.line(innerLeft, titleY + 2, innerLeft + titleW, titleY + 2);

  const scopeBoxW = innerW * HEADER_SCOPE_BOX_W_RATIO;
  const scopeBoxX = innerLeft + innerW - scopeBoxW;
  const scopeBoxY = logoY;
  drawCombinedScopeBox(
    doc,
    scopeBoxX,
    scopeBoxY,
    scopeBoxW,
    meta.scopeA || DEMO_SCOPE,
    meta.scopeB || DEMO_SCOPE
  );

  doc.setTextColor(0, 0, 0);
  return scopeBoxY + SCOPE_BOX_H + HEADER_TABLE_GAP;
}
...
export async function buildDefectComparisonReportPdfBlob(data = {}) {
  ...
  const startPage = (withFullHeader) => {
    if (withFullHeader) {
      y = drawFirstPageHeader(doc, logoDataUrl, meta);
    } else {
      y = V_MARGIN;
    }
