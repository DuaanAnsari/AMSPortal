...
/**
 * Inquiry Report — landscape PDF.
 *
 * Layout (per legacy print):
 *   - Top-LEFT  : AMS logo.
 *   - Top-CTR   : "INQUIRY" title (bold, 16pt, serif italic-ish via helvetica
 *                 bold to keep PDF embed simple).
 *   - Table     : 13 cols — SERIAL #, REQUEST DATE, CUSTOMER NAME, DUE DATE,
 *                 DISPATCH DATE, STYLE #, ITEM, CONTENT, FABRIC, COLOUR,
 *                 QTY, SIZE, GSM. The header strip is dark gray with white
 *                 text. DISPATCH DATE values render in bold.
 *   - Item card : Each item occupies a "card" that spans the full inner width.
 *                 Inside the card, the left 82 % carries the data row (table)
 *                 + a bordered Comment block (COMMENT STATUS, PROGRESS
 *                 COMMENTS, REMARKS) below it, and the right 18 % carries
 *                 the product image (placeholder rectangle when no URL is
 *                 attached; an embedded image otherwise).
...
/** @typedef {{ key: string; label: string; weight: number; align?: 'left'|'center'|'right' }} InquiryCol */
...
const COLS = [
  ...
  { key: 'colour', label: 'COLOUR', weight: 50, align: 'center' },
  { key: 'qty', label: 'QTY', weight: 25, align: 'center' },
  { key: 'size', label: 'SIZE', weight: 25, align: 'center' },
  { key: 'gsm', label: 'GSM', weight: 35, align: 'center' },
];
...
const RAW_DEMO_ITEMS = [
  ...
  },
  {
    serial: 'AMS/2942',
    requestDate: '01/02/2026',
    customerName: 'ALL SEASONS TEXTILE INC',
    dueDate: '12/19/2025',
    dispatchDate: '01/12/2026',
    styleNo: 'AMS/2942',
...
export const INQUIRY_REPORT_DEMO = {
...
function drawDataRow(doc, y, x0, widths, item) {
  ...
  COLS.forEach((col, i) => {
    const x = xs[i];
    const w = widths[i];
    // Light bottom rule only — keeps the row visually open like the legacy mock-up.
    ...
    /** Customer Name and Dispatch Date render in bold to match the legacy print. */
    const isBoldColumn = col.key === 'customerName' || col.key === 'dispatchDate';
...
function drawCommentBox(doc, y, x0, w, item) {
  ...
  // Row 1 — COMMENT STATUS  |  PROGRESS COMMENTS
  const row1Y = y;
  // Label
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7.4);
...
function drawImageCell(doc, x, y, w, h, item) {
  ...
  // Placeholder rectangle (lightly filled + bordered). When the backend
  // ships actual image URLs, swap this for a fetched data-URL via doc.addImage.
  if (item.imageUrl) {
    try {
...
function drawItemCard(doc, y, x0, innerW, item) {
  ...
  // Image cell spans the full card height on the right.
  drawImageCell(doc, imageX, y, imageW, cardH, item);

  // Left zone: data row at the top, comment box pinned to the bottom of the
  // card (with a generous vertical gap so the card breathes like the legacy
  // mock-up).
  ...
  // Full-width horizontal divider that separates this item from the next.
 ...
 * @param {Blob} pdfBlob
 */
export function openInquiryReportPdf(mode, pdfBlob) {
