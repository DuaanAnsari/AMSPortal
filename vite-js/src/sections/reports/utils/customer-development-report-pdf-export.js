...
/**
 * Customer Development Report — landscape PDF (Sample Development Report,
 * "ALL Customer" variant).
 *
 * Visual layout matches the legacy print mock-up:
 *   - Top      : "CUSTOMER DEVELOPMENT REPORT" centered in a bordered
 *                title strip.
 *   - Sub-bar  : "FROM : YYYY-MM-DD   TO : YYYY-MM-DD" centered, same
 *                border as the title.
 *   - Table    : 12 cols — S#, PICTURE (2 sub-cells), CUSTOMER,
 *                STYLE / DESCRIPTION, FABRIC / CONTENT / GSM, SIZE,
 *                COLORS, PRICE, FOB PRICE, QTY, DELIVERY DATE, STATUS.
 *                Header labels render bold + underlined to mirror the
 *                legacy ASP.NET print.
 *   - Item row : Multi-line fields (STYLE / DESCRIPTION, FABRIC /
 *                CONTENT / GSM) are stacked top-down inside the cell.
 *                Both PICTURE sub-cells render either an embedded image
 *                or the legacy "NO IMAGE AVAILABLE" placeholder.
...
const COLS = [
  ...
  { key: 'customer', label: 'CUSTOMER', weight: 92, align: 'left' },
...
/**
 * Build the Customer Development Report PDF blob.
 *
 * @param {{
 *   items?: object[];
 *   title?: string;
 ...
 */
export async function buildCustomerSdrPdfBlob(data = {}) {
  const payload =
    data && Array.isArray(data.items) && data.items.length > 0 ? data : CUSTOMER_SDR_DEMO;

  const meta = {
    title: payload.title || data.title || CUSTOMER_SDR_DEMO.title,
    fromDate: payload.fromDate || data.fromDate || CUSTOMER_SDR_DEMO.fromDate,
    toDate: payload.toDate || data.toDate || CUSTOMER_SDR_DEMO.toDate,
    printedOn: payload.printedOn || data.printedOn || formatPrintedOnLong(),
  };

  // Logo is fetched for parity with the rest of the Inquiry hub PDFs even
 ...
 * @param {Blob} pdfBlob
 */
export function openCustomerSdrPdf(mode, pdfBlob) {
  ...
  a.href = blobUrl;
  a.download = 'Customer-Development-Report.pdf';
  a.rel = 'noopener';
