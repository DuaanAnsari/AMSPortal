...
/**
 * Sample Development Report — CS Wise (Customer / Supplier wise) landscape PDF.
 *
 * Same overall layout as the dispatch variants but without the DISPATCH DATE
 * column and with an *empty* title strip on top (only the FROM/TO sub-bar
 * carries content), exactly per the legacy CS Wise print mock-up.
 *
 *   - Top      : empty bordered strip (placeholder for the print title).
 *   - Sub-bar  : "FROM : MM/DD/YYYY   TO : MM/DD/YYYY" centered.
 *   - Table    : 14 cols — S#, PICTURE, CUSTOMER / INQUIRY DATE, FTY NAME /
 *                HOD DATE, STYLE WITH DESCRIPTION, FABRIC / CONTENT / GSM,
 *                CATEGORY, SIZE, COLORS, QTY, FOB PRICE, FACTORY DEL. DATE,
 *                CUSTOMER DEL. DATE, STATUS.
 *   - Item row : Multi-line cells (CUSTOMER / INQUIRY DATE, FTY NAME /
 *                HOD DATE, STYLE, FABRIC) match the regular Sample
 *                Development Report layout.
 *
 * Demo data is hard-coded (1 row transcribed from the legacy mock-up);
...
const COLS = [
  ...
  { key: 'pictures', label: 'PICTURE', weight: 90, align: 'center', render: 'picture' },
  { key: 'customerBlock', label: 'CUSTOMER /\nINQUIRY DATE', weight: 78, align: 'left', render: 'stack2' },
  { key: 'factoryBlock', label: 'FTY NAME /\nHOD DATE', weight: 70, align: 'left', render: 'stack2' },
  { key: 'styleBlock', label: 'STYLE WITH\nDESCRIPTION', weight: 78, align: 'left', render: 'stack2' },
  { key: 'fabricBlock', label: 'FABRIC /\nCONTENT / GSM', weight: 86, align: 'left', render: 'stack3' },
  { key: 'category', label: 'CATEGORY', weight: 54, align: 'left' },
  ...
  { key: 'factoryDelDate', label: 'FACTORY\nDEL. DATE', weight: 58, align: 'center' },
  { key: 'customerDelDate', label: 'CUSTOMER\nDEL. DATE', weight: 58, align: 'center' },
  { key: 'status', label: 'STATUS', weight: 70, align: 'left' },
];

// ----------------------------------------------------------------------
// Demo data — transcribed from the legacy CS Wise print mock-up.
// ----------------------------------------------------------------------

const CS_WISE_SDR_DEMO = {
  title: '',
  fromDate: '01/01/2026',
  toDate: '12/31/2026',
  printedOn: null,
...
// ----------------------------------------------------------------------
// Title strip (empty per mock-up) + sub-title (date range)
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Picture cell — single sub-cell, image or "NO IMAGE AVAILABLE"
// ----------------------------------------------------------------------
...
/**
 * Build the Sample Development Report — CS Wise PDF blob.
 *
 * @param {{
 *   items?: object[];
 *   title?: string;
 ...
 * @param {Blob} pdfBlob
 */
export function openCsWiseSdrPdf(mode, pdfBlob) {
  ...
  a.href = blobUrl;
  a.download = 'Sample-Development-Report-CS-Wise.pdf';
  a.rel = 'noopener';
