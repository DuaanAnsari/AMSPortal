...
/**
 * Photo Shoot Sample (Factory) — landscape PDF.
 *
 * Visual layout matches the legacy "PHOTO SHOOT SAMPLE" factory print:
 *   - Top-LEFT       : AMS logo.
 *   - Title bar      : Bordered strip across the full table width with the
 *                      uppercase "PHOTO SHOOT SAMPLE" centered.
 *   - Table header   : S#, STYLE, DESCRIPTION, SIZE, PICTURE (4 picture
 *                      sub-cells), COLORS, FABRIC, GSM, PRICE, F.WASH,
 *                      FACTORY, QTY, STATUS. Header strip is light gray
 *                      with bold black text.
 *   - Item row       : All cell values are vertically centered. The 4
 *                      image cells render either an embedded image or the
 *                      legacy "NO IMAGE AVAILABLE" placeholder.
 ...
 *
 * Demo data is hardcoded to mirror the legacy factory mock-up (AYYOUB /
 * HASAN / ZAHID factories). Backend rows can be passed via the same shape
 * later.
 */
 ...
 *   isPicture?: boolean;
 * }} PhotoShootFactoryCol
 */

/**
 * Column model. Compared to the customer-side Photo Shoot Sample PDF this
 * variant inserts two extra columns:
 *   - `PRICE`   after `GSM`.
 *   - `FACTORY` after `F.WASH`.
 *
 * @type {PhotoShootFactoryCol[]}
 */
const COLS = [
  { key: 'serial', label: 'S#', weight: 22, align: 'center' },
  { key: 'styleNo', label: 'STYLE', weight: 46, align: 'center' },
  { key: 'description', label: 'DESCRIPTION', weight: 66, align: 'center' },
  { key: 'size', label: 'SIZE', weight: 26, align: 'center' },
  { key: 'pictures', label: 'PICTURE', weight: 260, align: 'center', isPicture: true },
  { key: 'colors', label: 'COLORS', weight: 50, align: 'center' },
  { key: 'fabric', label: 'FABRIC', weight: 70, align: 'center' },
  { key: 'gsm', label: 'GSM', weight: 28, align: 'center' },
  { key: 'price', label: 'PRICE', weight: 30, align: 'center' },
  { key: 'fWash', label: 'F.WASH', weight: 32, align: 'center' },
  { key: 'factory', label: 'FACTORY', weight: 60, align: 'center' },
  { key: 'qty', label: 'QTY', weight: 26, align: 'center' },
  { key: 'status', label: 'STATUS', weight: 54, align: 'center' },
];

// ----------------------------------------------------------------------
// Demo data — transcribed from the legacy "PHOTO SHOOT SAMPLE" factory mock-up.
// ----------------------------------------------------------------------

const PHOTO_SHOOT_SAMPLE_FACTORY_DEMO = {
  printedOn: null,
  items: [
    {
      serial: '1',
    ...
    },
    {
      serial: '5',
      styleNo: 'AMS/2945',
      description: 'COLOR BLOCK SUBLIMATED PRINTED CREW',
      size: 'L',
      pictures: [],
      colors: 'BLUE/VINTAGE',
      fabric: 'FLEECE\n\n50% COTTON\n50% POLYESTER',
    ...
    },
    {
      serial: '6',
      styleNo: 'AMS/2949',
      description: 'MEN,S CREW NECK TEE',
      size: 'M',
      pictures: [],
      colors: 'BLACK',
      fabric: 'JERSEY\n\n100%COTTON',
    ...
    },
    {
      serial: '7',
      styleNo: 'AMS/2950',
      description: 'MEN,S SWEATSHIRT DTF',
      size: 'L',
      pictures: [],
      colors: 'BLACK',
      fabric: 'FLEECE\n\n60% COTTON\n40% POLYESTER',
    ...
    },
    {
      serial: '8',
      styleNo: 'AMS/2951',
      description: 'MEN,S CREW NECK DTF',
      size: 'M',
      pictures: [],
      colors: 'BLACK',
      fabric: 'JERSEY\n\n100%COTTON',
...
export { PHOTO_SHOOT_SAMPLE_FACTORY_DEMO };
...
// ----------------------------------------------------------------------
// Header band — logo + "PHOTO SHOOT SAMPLE" title bar
// ----------------------------------------------------------------------

function drawHeaderBand(doc, logoDataUrl, tableX, tableW) {
  if (logoDataUrl) {
    try {
      const logoW = 100;
      const logoH = 32;
  ...
  doc.setFontSize(11);
  doc.setTextColor(0, 0, 0);
  doc.text('PHOTO SHOOT SAMPLE', tableX + tableW / 2, titleY + TITLE_BAR_H / 2, {
    align: 'center',
    baseline: 'middle',
  });
...
/**
 * Build the Factory Photo Shoot Sample PDF blob.
 *
...
export async function buildPhotoShootSampleFactoryPdfBlob(data = {}) {
  ...
  /**
   * Start a fresh page: header band (logo + title) + table header.
   * @returns {number} the y position where item rows can begin.
   */
  const startPage = () => {
    const afterHeader = drawHeaderBand(doc, logoDataUrl, tableX, tableW);
 ...
 * @param {Blob} pdfBlob
 */
export function openPhotoShootSampleFactoryPdf(mode, pdfBlob) {
  ...
  a.href = blobUrl;
  a.download = 'Photo-Shoot-Sample-Factory.pdf';
  a.rel = 'noopener';
