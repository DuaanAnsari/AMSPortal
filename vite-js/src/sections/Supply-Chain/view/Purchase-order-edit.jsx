...
function FileUploadWithPreview({ name, label, accept = "image/*" }) {
  ...
  const isImage = typeof fileValue === 'string'
    ? (fileValue.match(/\.(jpeg|jpg|gif|png)$/i) != null || fileValue.startsWith('data:image'))
    : fileValue?.type?.startsWith('image/');

  const isPdfOrDoc = typeof fileValue === 'string' && fileValue.match(/\.(pdf|doc|docx)$/i) != null;
  return (
    <Box>
      <Button
        variant="outlined"
        ...
        <Box sx={{ mt: 1, p: 1, border: '1px solid #ddd', borderRadius: 1 }}>
          ...
          {isPdfOrDoc && (
            <Box sx={{ mt: 2, textAlign: 'center' }}>
              <Button variant="outlined" component="a" href={fileValue} target="_blank" rel="noopener noreferrer" startIcon={<UploadFileIcon />}>
...
export default function CompletePurchaseOrderFormEdit() {
      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          const getFileUrl = (data, defaultName, originalName) => {
            ...
            return `${baseUrl}/POFiles/${id}/${fileName}`;
          };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Convert new File objects to base64 and add to payload
      const processFile = async (fileObj) => {
        if (fileObj instanceof File) {
          return await fileToBase64(fileObj);
        }
        return null;
      };
      ...
      if (data.finalSpecs instanceof File) {
        payload.finalspecs = await processFile(data.finalSpecs);
      }
      if (data.productImage instanceof File) {
        payload.productImage = await processFile(data.productImage);
        payload.prodImgFileName = data.productImage.name;
      }
      if (data.ppComment instanceof File) {
        payload.pPimage = await processFile(data.ppComment);
      }
      if (data.sizeSetComment instanceof File) {
        payload.sizeset = await processFile(data.sizeSetComment);
      }
