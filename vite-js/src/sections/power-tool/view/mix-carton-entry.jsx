...
export default function MixCartonEntryPage() {
    ...
    const handleShowData = useCallback(async () => {
        ...
        try {
            ...
            const mapped = (Array.isArray(raw) ? raw : []).map((item, index) => ({
                ...
                styleNo: item.styleno ?? item.styleNo ?? item.StyleNo ?? item.StyleNO ?? item.style ?? item.Style ?? '',
                colorway: item.colorway ?? item.Colorway ?? item.colorWay ?? item.ColorWay ?? item.color ?? item.Color ?? '',
                color: item.colorway ?? item.Colorway ?? item.colorWay ?? item.ColorWay ?? item.color ?? item.Color ?? '',
    ...
    const columns = [
        ...
        { field: 'colorway', headerName: 'Colorway', flex: 1.5, valueGetter: (params) => params?.row?.colorway ?? params?.row?.color ?? '' },
